...
class ShowComponents {
	...
	public function Add() {
		...
		if(isset($_POST['submit']) or isset($_POST['update'])) {
			...
			$smd				=	strip_tags(mysqli_real_escape_string($connection,$_POST['smd']));
			//$public				=	strip_tags(mysqli_real_escape_string($_POST['public']));
			//$width				=	str_replace(',', '.', strip_tags(mysqli_real_escape_string($connection,$_POST['width'])));
			//$height				=	str_replace(',', '.', strip_tags(mysqli_real_escape_string($connection,$_POST['height'])));
			//$depth				=	str_replace(',', '.', strip_tags(mysqli_real_escape_string($connection,$_POST['depth'])));
			//$weight				=	str_replace(',', '.', strip_tags(mysqli_real_escape_string($connection,$_POST['weight'])));
			...
			//$appnote2				=	strip_tags(mysqli_real_escape_string($connection,$_POST['appnote2']));
			//$url4				=	strip_tags(mysqli_real_escape_string($connection,$_POST['url4']));
...
/*			elseif (!empty($_POST['weight']) && !is_numeric($weight)) {
			...
			*/
				...
				if(isset($_POST['submit'])) {
					$sql="INSERT into data (owner, name, manufacturer, package, pins, smd, quantity, location, scrap, datasheet, comment, category, cimage,appnote, price, order_quantity)
					VALUES ('$owner', '$name', '$manufacturer', '$package', '$pins', '$smd', '$quantity', '$location', '$scrap', '$datasheet', '$comment', '$category', '$cimage', '$appnote' ,'$price', '$order_quantity')";
					...
					foreach ($proj as $quantity){
						...
						if ($quantity == 0){
							...
							$sql_exec = mysqli_query($proj_add);
				...
				if(isset($_POST['update'])) {
					$sql = "UPDATE data SET
					name = '$name', manufacturer = '$manufacturer', package = '$package', pins = '$pins', smd = '$smd', quantity = '$quantity', location = '$location',	scrap = '$scrap', datasheet = '$datasheet', comment = '$comment', category = '$category', cimage = '$cimage', price = '$price', order_quantity = '$order_quantity' ,appnote = '$appnote' 	WHERE id = '$id'";
