...
class ProjectShowComponents {	
	public function ProjectShowComponents() {
		...
		if(isset($_GET['proj_id'])) {
			...
			$proj_id = mysqli_real_escape_string($connection,(int)$_GET['proj_id']);
			
			$GetComponentIDs = "SELECT component_id FROM projects_data WHERE owner = ".$owner." AND project_id = ".$proj_id." ORDER by name ASC";
			$sql_exec_GetComponentIDs = mysqli_Query($connection,$GetComponentIDs);
			...
			while($showDetails = mysqli_fetch_array($sql_exec)) {
				...
				echo '><span class="fa fa-pencil fa-lg"/></a></td>';
				...
				if ($datasheet==""){
				    ...
				    echo '"  target="_blank"><span class="fa fa-document fa-lg"/></a></td>';
				}
				...
				if ($comment==""){
					...
					echo '<td class="comment"><div><span class="fa fa-comment fa-lg"/></span><span>';
